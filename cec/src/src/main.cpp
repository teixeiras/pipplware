...
const std::vector<list<__u16>> & Main::setupUinputMap() {
	...
	if (uinputCecMap.empty()) {
		...
		std::vector< pair<int, list<keyType> > > mapFromXML = parser->xmlKeyList();
		for(std::vector< pair<int, list<keyType> > >::iterator it = mapFromXML.begin(); it != mapFromXML.end(); ++it) {
			pair<int, list<keyType> > map = *it;
			cout<<"Redifining " << map.first;
			uinputCecMap[map.first] = map.second;
		}
	}
